...
static int dash_read(struct fastchg_device_info *di)
{
	...
	if (data == 0x54) {
		pr_err("start dash recovery if not unplugged\n");
		set_property_on_smbcharger(POWER_SUPPLY_PROP_START_DPDM_RECOVERY, true);
	} else if (data == 0x52) {
		pr_err("stop dash recovery\n");
		set_property_on_smbcharger(POWER_SUPPLY_PROP_START_DPDM_RECOVERY, false);
	}
	return data;
}
