...
#define GOODIX_TOOLS_MAX_DATA_LEN      16*1024
...
static int async_read(void __user *arg)
{
    ...
    if (touch_i2c_read_block_u32(gtx8_tool_info.client, reg_addr, length, databuf) >= 0) {
...
static int async_write(void __user *arg)
{
    ...
    if (touch_i2c_write_block_u32(gtx8_tool_info.client, reg_addr, length, databuf) < 0) {
        ret = -EBUSY;
