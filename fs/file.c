...
void fd_show_open_files(pid_t pid, struct files_struct *files, struct fdtable *fdt)
{
	...
	for (i = 0; i < fdt->max_fds; i++) {
		...
		if (!entry) {
			pr_err("[FD_LEAK] PID:%d Empty FD:%d", pid, i);
		} else {
	...
	for (;;) {
		...
		if (lentry != NULL) {
			...
			if (lentry->name[0])
				pr_err("[FD_LEAK]OverAllocFDError(PID:%d fileName:%s Num:%d)\n",
	...
	if (sum_fds_of_pid)
		pr_err("[FD_LEAK]OverAllocFDError(PID:%d totalFDs:%d)\n",
			pid, sum_fds_of_pid);
...
repeat:
	...
	if (rcu_access_pointer(fdt->fd[fd]) != NULL) {
		pr_err("alloc_fd: slot %d not NULL!\n", fd);
...
out:
	...
	if (error == -EMFILE) {
		...
		if (current->group_leader->dump_fd_leak == false) {
			...
			pr_err("[FD_LEAK](PID:%d)fd over RLIMIT_NOFILE:%ld",
